...
3. Then, in the project folder (still from your terminal), let's build the images with `podman build -t simple-api-image .`
4. Once it has finished and the images successfully built, you should be able to see it by typing `podman images`
5. Finally, you can run the container by typing `podman run -d --name simple-api-container -p 4000:4000 simple-api`
